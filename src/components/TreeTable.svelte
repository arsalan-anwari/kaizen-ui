  ...
  <table class="w-full border-separate border-spacing-y-1 text-sm {className}">
    <thead>
      <tr class="text-start text-[0.625rem] font-bold tracking-wide text-muted-foreground uppercase">
        ...
        <th class="py-1 pe-2 text-end font-bold">{columns.score}</th>
      </tr>
    </thead>
    <tbody>
      {#each groups as group (group.key)}
        <tr class="bg-surface">
          <td class="rounded-s-lg py-2 ps-2">
            <button
              type="button"
          ...
          <td class="px-2 py-2 text-end tabular-nums">{percent(group)}%</td>
          <td class="hidden px-2 py-2 sm:table-cell">
            <Meter value={share(group)} tone="heat" size="sm" showValue={false} />
          </td>
          <td class="rounded-e-lg py-2 pe-2 text-end tabular-nums text-muted-foreground">
        ...
        {#if !closed.has(group.key)}
          {#each group.children as child (child.key)}
            <tr>
              <td class="py-1.5 ps-6 text-muted-foreground">
                <span class="flex items-center gap-1.5">
                  <span class="size-1 shrink-0 rounded-full bg-muted-foreground"></span>
                  {child.label}
                </span>
              </td>
              <td class="px-2 py-1.5 text-end text-xs tabular-nums">{percent(child)}%</td>
              <td class="hidden px-2 py-1.5 sm:table-cell">
                <Meter value={share(child)} tone="heat" size="sm" showValue={false} />
              </td>
              <td class="py-1.5 pe-2 text-end text-xs tabular-nums text-muted-foreground">
